...
st.title('**Welcome!**')
st.write('This app displays stock price predictions using data from Yahoo! Finance. Choose a stock symbol as listed on the [Yahoo! Finance website](https://finance.yahoo.com/lookup/).')
#------------------------------------------------------------------------------#
...
#         forecast_df = forecast_df[forecast_df['Date'] <= end_date]
        ...
        st.write("Error: An error occurred during model fitting. Check your columns and try again.")
        return None
...
st.subheader("Your Predictions start here")
st.write('Choose a Stock Symbol')
...
if ticker:
    ...
    end_date = st.sidebar.date_input("End Date", pd.to_datetime("2023-12-19"))
...
#         forecast_df = forecast_df[forecast_df['Date'] <= end_date]
        ...
        if st.button('Save Your Predictions'):
            ...
            csv_data = forecast_df.to_csv(index=True).encode()
